...
namespace Gogaman
{
	...
	{
	public:
		static Shader LoadShader(const std::string &name, const char *vertexShaderPath, const char *fragmentShaderPath, const char *geometryShaderPath = nullptr);
		static Shader LoadShader(const std::string &name, const char *computeShaderPath);
		inline static Shader GetShader(const std::string &name) { return shaders[name]; }

		static Texture2D LoadTexture2D(const std::string &name, const char *filePath, const GLboolean &sRGB);
		inline static Texture2D GetTexture2D(const std::string &name) { return texture2Ds[name]; }

		static Model LoadModel(const std::string &name, const char *filePath);
