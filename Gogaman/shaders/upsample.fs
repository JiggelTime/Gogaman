...
void  TightenAABBchroma(inout vec3 aabbAvg, inout vec3 aabbMin, inout vec3 aabbMax, vec2 currentPixelChroma);
...
float TapWeight(vec3 centerN, vec3 sampleN, float centerZ, float sampleZ, float centerL, float sampleL, vec2 offset, float centerZGradient);
vec4  SpatialResolve(sampler2D inputTexture, sampler2D depthTexture, sampler2D depthCoarseTexture, sampler2D normalTexture, sampler2D normalCoarseTexture);
...
void main()
{
	vec4 result = SpatialResolve(inputTexture, depthTexture, depthCoarseTexture, normalTexture, normalCoarseTexture);
	//vec4 result = TemporalResolve(inputTexture, previousInputTexture);

	//if(debug2) result.rgb = vec3(LinearizeDepth(textureLod(depthCoarseTexture, texCoordsFrag, sampleTextureLod).x));
	//if(debug2) result.rgb = DecodeSignedOctahedronNormal(textureLod(normalCoarseTexture, texCoordsFrag, sampleTextureLod).xy);
...
void TightenAABBchroma(inout vec3 aabbAvg, inout vec3 aabbMin, inout vec3 aabbMax, vec2 currentPixelChroma)
{
	...
	vec2 chromaCenter = currentPixelChroma;
...
vec4 TemporalResolve(sampler2D inputTexture, sampler2D previousInputTexture)
{
	...
	TightenAABBchroma(neighborhoodAvg.xyz, neighborhoodMin.xyz, neighborhoodMax.xyz, currentPixelValue.yz);
...
float TapWeight(vec3 centerN, vec3 sampleN, float centerZ, float sampleZ, float centerL, float sampleL, vec2 offset, float centerZGradient)
{
	...
	float weightL = 1.0f;
	float weight  = weightN * weightZ * weightL;
	return weight;
}

vec4 SpatialResolve(sampler2D inputTexture, sampler2D depthTexture, sampler2D depthCoarseTexture, sampler2D normalTexture, sampler2D normalCoarseTexture)
{
	vec4  centerValue         = texture(inputTexture, texCoordsFrag);
	...
	float centerLuma          = Luminance(centerValue.rgb);
	...
	for(int i = -SPATIAL_KERNEL_RADIUS; i <= SPATIAL_KERNEL_RADIUS; i++)
	{
		for(int j = -SPATIAL_KERNEL_RADIUS; j <= SPATIAL_KERNEL_RADIUS; j++)
		{
			...
			vec4  sampleValue  = textureOffset(inputTexture, texCoordsFrag, sampleOffset);
			...
			float sampleWeight = TapWeight(centerNormal, sampleNormal, centerDepth, sampleDepth, centerLuma, sampleLuma, sampleOffset, centerDepthGradient);
	...
	result /= weightTotal;
	//This is not entirely necessary. Helps when there are no coherent samples found. In practice, this rarely occurs.
